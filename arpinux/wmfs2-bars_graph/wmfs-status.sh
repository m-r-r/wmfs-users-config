...
cpugraph(){
    ...
    echo "^s[140;10;$grey;core]^g[170;2;80;8;$cpu;100;$dark;$orange;cpugraph](1;spawn;urxvtc -e htop)"
}
# membar
membar(){
    memu="$(free -m | sed -n 's|^-.*:[ \t]*\([0-9]*\) .*|\1|gp')"
    memt="$(free -m | sed -n 's|^M.*:[ \t]*\([0-9]*\) .*|\1|gp')"
    echo "^s[260;10;$grey;mem ]^p[290;2;80;8;0;$memu;$memt;$dark;$lightgreen](1;spawn;urxvtc -e htop)^R[330;2;1;10;$default]"
}
# hdd section
hdd(){
    hdd="$(df -h|grep sda3|awk '{print $5}' | cut -c1-2)"
    echo "^s[390;10;$grey;hdd]^p[415;2;40;8;0;$hdd;100;$dark;$grey](1;spawn;urxvt -e ncdu)"
}
# internet section
netup(){
    netup="$("$HOME/bin/speedup.sh")" 
    echo "^s[465;10;$grey;net ]^p[490;2;100;4;0;$netup;150;$dark;$dblue](1;spawn;urxvt -e net-monitor)" 
}
netdown(){ 
    netdown="$("$HOME/bin/speeddown.sh")" 
    echo "^p[490;7;100;4;0;$netdown;2200;$dark;$blue](1;spawn;urxvt -e net-monitor)" 
}
# date/time section
day(){
    day="$(date +"%d")"
...
statustext()
{
    wmfs -c status "leftbar $(email) $(pwr) $(cpugraph) $(membar) $(hdd) $(netup) $(netdown) $(day) $(month) $(hour) $(minut)"
     wmfs -c status "rightbar $(volpcm) $(volibm) $(mocpctrl) $(mocpbar)$(music)"
